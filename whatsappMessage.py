...
def SendEmail(info):
    ...
    for i in pump_keys:
        if i == Address:
            ...
            Telegram_Message = "https://api.telegram.org/bot1733010924:(your bot token)/sendMessage?chat_id=(yourgroup)&text=Kayaba%20Crypto%20Bot%0A%0A--TRANSACTION%20ALERT!!!--%0A%0A---{name}%20{action}---%0A%0A%0AToken:%20%20{Token}%20For%20${value}%0A%0ATransaction%20Receipt:{transaction}%0AToken%20Contract:{TC}%0A{Token}%20Chart:{chart}%0A%0A%20Any%20Donations%20are%20welcome%20:)%20%20--0xd4ec9a0BCd9D1Aeb295c0412641d15095A0C002F--".format(name=pump_name,action=Action,Token = Token, value = DOLLARS,transaction=Transaction,TC=TContract,chart=Chart)
